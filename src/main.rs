use std::{collections::{HashMap, HashSet, VecDeque}, hash::Hash, io::{self, BufRead, BufReader, Read}, iter};
use std::fs::File;
...
fn main() {
    ...
    */
    ...
    let part1_1 = part1_1(&lines);
    let part2 = part2_2(&lines);
    println!("Part 1: {part1}");
    println!("Part 1_1: {part1_1}");
            ...
            for dest_module_name in output_module_names {
                ...
                if let Some(dest_module_connections) = connections_map.get(dest_module_name) {
                    match dest_module_connections {
                        ...
                        PulseModule::Conjunction(inputs, outputs) => {
                            let dest_new_state;
                            if inputs.iter().all(|(module_name, _)| match state_map.get(module_name).unwrap() { Signal::High => true, Signal::Low => false } ) {
                ...
                if state_changed_flag {
                    module_queue.push_back(dest_module_name.as_str());
                }
            }
        }
    }
...
fn part1_1(lines: &Vec<String>) -> i32 {
    // Parse input
    let mut parsed_lines = Vec::new();
    for ln in lines {
        let (module_str, output_list_str) = ln.split_once(" -> ").unwrap();
...
fn part2(lines: &Vec<String>) -> i32 {    // Parse input
    ...
    */
    ...
    'button_press_loop: for i in 0..5000 {
            ...
            for dest_module_name in output_module_names {
                ...
                if let Some(dest_module_connections) = connections_map.get(dest_module_name) {
                    match dest_module_connections {
                        ...
                        PulseModule::Conjunction(inputs, outputs) => {
                            let dest_new_state;
                            if inputs.iter().all(|(_, sgl)| *sgl == Signal::High) {
                                dest_new_state = Signal::Low;
                            }
                            else {
                                dest_new_state = Signal::High;
...
fn iterate_subgraph(connections_map: &HashMap<String, PulseModule>, state_map: &mut HashMap<String, Signal>, conj_mem_map: &mut HashMap<String, HashMap<String, Signal>>, subgraph: &HashSet<String>) -> (i32, i32) {
    ...
    while let Some(source_module_name) = module_queue.pop_front() {
        ...
        // Change state in output modules
        for dest_module_name in output_module_names {
            if (source_module_name == "ch" && source_module_state == Signal::High) {
                println!("ch emits High");
            }
